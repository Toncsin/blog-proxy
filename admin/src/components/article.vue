...
<script>
  export default {
    ...
    beforeCreate:function(){
      this.$http.get('/list/article/find').then((result)=>{
        this.article=result.body;
      })
    },
    methods:{
      updshow:function(e){
      ...
      updsubmit:function(){
        this.$http.get('/list/article/update?id='+this.id+"&title="+this.newtitle+"&content="+this.newcontent).then(result=>{
          if(result.body.code==1){
            alert('更新成功!');
            this.$router.replace('/blank2')
          }
        });
      ...
      addsubmit:function(){
        this.$http.get('/list/article/insert?title='+this.newtitle+"&content="+this.newcontent).then(result=>{
          if(result.body.code==1){
            alert('添加成功!');
            this.$router.replace('/blank2')
          }
      ...
      deletes:function(e){
        if(confirm("确定删除?")){
          this.id=e.target.parentNode.parentNode.firstChild.innerHTML;
          this.$http.get('/list/article/delete?id='+this.id).then(result=>{
            if(result.body.code==1){
              alert('删除成功!');
              this.$router.replace('/blank2')
            }
