...
<script>
    export default {
      ...
      beforeCreate:function(){
        this.$http.get('/list/essay/find').then((result)=>{
          this.essay=result.body;
        })
      },
      methods:{
        updshow:function(e){
        ...
        updsubmit:function(){
          this.$http.get('/list/essay/update?id='+this.id+"&title="+this.newtitle+"&content="+this.newcontent).then(result=>{
            if(result.body.code==1){
              alert('更新成功!');
              this.$router.replace('/blank1')
            }
          });
        ...
        addsubmit:function(){
          this.$http.get('/list/essay/insert?title='+this.newtitle+"&content="+this.newcontent).then(result=>{
            if(result.body.code==1){
              alert('添加成功!');
              this.$router.replace('/blank1')
            }
        ...
        deletes:function(e){
          if(confirm("确定删除?")){
          this.id=e.target.parentNode.parentNode.firstChild.innerHTML;
           this.$http.get('/list/essay/delete?id='+this.id).then(result=>{
             if(result.body.code==1){
               alert('删除成功!');
               this.$router.replace('/blank1')
            }
